        ...
        if (!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i=0;i<allCounties.length();i++) {
                    JSONObject countyrObiect =allCounties.getJSONObject(i);
                    County county=new County();
                    county.setCountyName(countyrObiect.getString("name"));
                    county.setWeatherId(countyrObiect.getString("weather_id"));
